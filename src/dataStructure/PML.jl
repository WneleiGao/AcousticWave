...
function InitCpml(nz::Int64, nx::Int64, ext::Int64, vmax::Float64, dz::Float64, dx::Float64, iflag::Int64)
      ...
      if iflag == 1
         ...
         #  upper and left
         for i = 1 : ext
             vz[i,:] = -1.5 * vmax/(ext*dz) * log(R) *  ((ext+0.5-i)/ext)^ll
             vx[:,i] = -1.5 * vmax/(ext*dx) * log(R) *  ((ext+0.5-i)/ext)^ll
             pz[i,:] = -1.5 * vmax/(ext*dz) * log(R) *  ((ext+1  -i)/ext)^ll
             px[:,i] = -1.5 * vmax/(ext*dx) * log(R) *  ((ext+1  -i)/ext)^ll
         end
         #  lower
         for iz = nz+ext+1 : Nz
             vz[iz,:] = -1.5 * vmax/(ext*dz) * log(R) * ((iz+0.5-nz-ext)/ext)^ll
             pz[iz,:] = -1.5 * vmax/(ext*dz) * log(R) * ((iz    -nz-ext)/ext)^ll
         end
         #  right
         for ix = nx+ext+1 : Nx
             vx[:,ix] = -1.5 * vmax/(ext*dx) * log(R) * ((ix+0.5-nx-ext)/ext)^ll
      ...
      elseif iflag == 2
         ...
         #  lower
         for iz = nz+1 : Nz
             vz[iz,:] = -1.5 * vmax/(ext*dz) * log(R) * ((iz+0.5-nz)/ext)^ll
             pz[iz,:] = -1.5 * vmax/(ext*dz) * log(R) * ((iz    -nz)/ext)^ll
         end
         #  left
         for ix = 1 : ext
             vx[:,ix] = -1.5 * vmax/(ext*dx) * log(R) * ((ext+0.5-ix)/ext)^ll
...
# support changing density
# function InitCpml_pl(nz::Int64, nx::Int64, ext::Int64, vmax::Float64, rho::Array{Float64,2}, iflag::Int64)
...
#        #  upper and left
#        for i = 1 : ext
#            vz[i,:] = -vmax/ext * log(R) * (m*(ext+0.5-i)/ext + n*((ext+0.5-i)/ext)^ll) * rho[i,:]
#            vx[:,i] = -vmax/ext * log(R) * (m*(ext+0.5-i)/ext + n*((ext+0.5-i)/ext)^ll) * rho[:,i]
#            pz[i,:] = -vmax/ext * log(R) * (m*(ext+1  -i)/ext + n*((ext+1  -i)/ext)^ll)
#            px[:,i] = -vmax/ext * log(R) * (m*(ext+1  -i)/ext + n*((ext+1  -i)/ext)^ll)
#        end
#        #  lower
#        for iz = nz+ext+1 : Nz
#            vz[iz,:] = -vmax/ext * log(R) * (m*(iz+0.5-nz-ext)/ext + n*((iz+0.5-nz-ext)/ext)^ll) * rho[iz,:]
#            pz[iz,:] = -vmax/ext * log(R) * (m*(iz    -nz-ext)/ext + n*((iz    -nz-ext)/ext)^ll)
#        end
#        #  right
#        for ix = nx+ext+1 : Nx
#            vx[:,ix] = -vmax/ext * log(R) * (m*(ix+0.5-nx-ext)/ext + n*((ix+0.5-nx-ext)/ext)^ll) * rho[:,ix]
...
#        #  lower
#        for iz = nz+1 : Nz
#            vz[iz,:] = -vmax/ext * log(R) * (m*(iz+0.5-nz)/ext + n*((iz+0.5-nz)/ext)^ll) * rho[iz,:]
#            pz[iz,:] = -vmax/ext * log(R) * (m*(iz    -nz)/ext + n*((iz    -nz)/ext)^ll)
#        end
#        #  left
#        for ix = 1 : ext
#            vx[:,ix] = -vmax/ext * log(R) * (m*(ext+0.5-ix)/ext + n*((ext+0.5-ix)/ext)^ll) * rho[:,ix]
